...
<form>
	...
	<div class="col-lg-4 col-md-4 col-sm-12"><p>The Check In for this Book is Successful! There is a fine against this transaction which the borrower has to pay now OR later.</p><p>Details of the Borrower:  <br>
	<%	
		
 		if(session.getAttribute("loanIdCheckIn") != null){
	 		String borrowerId = (String) session.getAttribute("borrowerIdCheckIn");
	 		
	 		String bookId = (String) session.getAttribute("bookIdCheckIn");
	 		String bookLoanId = (String) session.getAttribute("loanIdCheckIn");
	 		/*  out.print(bookLoanId); */
	 		if(bookLoanId != null){
				int pageid = 1;
				int total = 5;
				if (pageid == 1) {
				...
				String borrowerName = getName.getName(borrowerId);
				 BookLoanFines list= FinesDAO.getSummaryForSingleBookFineDetails(bookLoanId);
...
/* 				 out.print("<table class='table table-striped table-condensed' border=1 style='margin-top: 10px'>");
				...
				out.print("<tr><th>Card Number : </th><td>" + borrowerId+ "</td></tr>");
				...
				out.print("<tr><td>" + list.getBook_id()+ "</td><td>" +list.getBook_name()+ "</td><td>"+list.getDate_out()
				+ "</td><td>" + list.getDue_date()+ "</td><td> $ " + list.getFine_amount()+ "</td><td> <a href='payfine?bookLoanId="+list.getBook_loan_id()+"' class=\"btn btn-primary\"> Pay Fine </td></tr>");
